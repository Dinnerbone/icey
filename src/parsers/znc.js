        ...
        this.patterns = [
            {
                ...
                result: (match, date) => this.counter.onAction(`${date} ${match.time}`, match.nick, match.action),
            },
            {
                pattern: XRegExp.build('^{{time}} <(?<nick>[^>]+)> (?<message>.*)$', subs),
                result: (match, date) => this.counter.onMessage(`${date} ${match.time}`, match.nick, match.message),
            },
            {
                pattern: XRegExp.build('^{{time}} {{system}} Joins: (?<nick>\\S+) {{hostmask}}$', subs),
                result: (match, date) => this.counter.onJoin(`${date} ${match.time}`, match.nick),
            },
            {
                pattern: XRegExp.build('{{time}} {{system}} (?<victimNick>\\S+) was kicked by (?<nick>\\S+) \\((?<message>.+)\\)$', subs),
                result: (match, date) => this.counter.onKick(`${date} ${match.time}`, match.nick, match.victimNick, match.message),
            },
            {
                pattern: XRegExp.build('^{{time}} {{system}} (?<nick>\\S+) sets mode: (?<modes>.+)$', subs),
                result: (match, date) => this.counter.onMode(`${date} ${match.time}`, match.nick, match.modes),
            },
            {
                pattern: XRegExp.build('{{time}} {{system}} (?<nick>\\S+) is now known as (?<newNick>\\S+)$', subs),
                result: (match, date) => this.counter.onNick(`${date} ${match.time}`, match.nick, match.newNick),
            },
            {
                pattern: XRegExp.build('^{{time}} -(?<nick>[^-]+)- (?<message>.*)$', subs),
                result: (match, date) => this.counter.onNotice(`${date} ${match.time}`, match.nick, match.message),
            },
            {
                pattern: XRegExp.build('^{{time}} {{system}} Parts: (?<nick>\\S+) {{hostmask}} \\((?<message>.*)\\)$', subs),
                result: (match, date) => this.counter.onPart(`${date} ${match.time}`, match.nick, match.message),
            },
            {
                pattern: XRegExp.build('^{{time}} {{system}} Quits: (?<nick>\\S+) {{hostmask}} \\((?<message>.*)\\)$', subs),
                result: (match, date) => this.counter.onQuit(`${date} ${match.time}`, match.nick, match.message),
            },
            {
                pattern: XRegExp.build('^{{time}} {{system}} (?<nick>\\S+) changes topic to \'(?<topic>.*)\'$', subs),
                result: (match, date) => this.counter.onTopic(`${date} ${match.time}`, match.nick, match.topic),
            },
        ];
    }
